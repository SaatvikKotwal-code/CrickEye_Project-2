...
const CrickEyeLengthInsights = (() => {
  ...
  function vsSessionPillHtml(execD) {
    ...
    const suffix = ` · ${num}`;
    if (execD > 0.1) {
      return `<span class="rp-length-vs-pill rp-length-vs-pill--green">▲ Better than avg${suffix}</span>`;
    }
    return `<span class="rp-length-vs-pill rp-length-vs-pill--red">▼ Needs work${suffix}</span>`;
  }

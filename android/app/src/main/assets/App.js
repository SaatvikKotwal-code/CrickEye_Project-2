...
// ── Mobile & Wireless Server Configuration ──────────────────
...
function getCandidateServerUrls() {
  ...
  if (saved && saved.trim()) {
    list.push(saved.trim().replace(/\/+$/, ''));
  }
  if (window.CRICKEYE_DEFAULT_BACKEND_URL && window.CRICKEYE_DEFAULT_BACKEND_URL.trim()) {
    list.push(window.CRICKEYE_DEFAULT_BACKEND_URL.trim().replace(/\/+$/, ''));
  }
  // USB reverse proxy port (instant & stable via USB adb reverse)
  list.push('http://localhost:8000');
  // PC LAN Wi-Fi (direct wireless access for phone on same Wi-Fi)
  list.push('http://10.253.13.192:8000');
...
function getApiBaseUrl() {
  ...
  return candidates[0] || 'http://10.253.13.192:8000';
}


function triggerHaptic(durationMs = 80) {
  const enabled = localStorage.getItem('crickeye_haptics') !== 'false';
  if (!enabled) return;
  try {
...
function triggerHapticPattern(type = 'countdown') {
  const enabled = localStorage.getItem('crickeye_haptics') !== 'false';
  if (!enabled) return;
  try {
    if (window.AndroidBridge && typeof window.AndroidBridge.vibratePattern === 'function') {
      window.AndroidBridge.vibratePattern(type);
      return;
    }
    if (type === 'countdown') {
      triggerHaptic(60);
      ...
      if (navigator.vibrate) navigator.vibrate([100, 100, 150]);
      else triggerHaptic(120);
    } else {
      triggerHaptic(80);
    }
  } catch (e) {}
...
function setScreenWakeLock(enable) {
  try {
    if (window.AndroidBridge && typeof window.AndroidBridge.setKeepScreenOn === 'function') {
      window.AndroidBridge.setKeepScreenOn(Boolean(enable));
    }
  } catch (e) {}
}

// ── Physical / System Back Button Navigation (Android Native) ──────
window.handleAndroidBack = function() {
  const openModals = Array.from(document.querySelectorAll('.ce-modal:not([hidden])'));
  ...
  if (reportModal && !reportModal.hidden && reportModal.style.display !== 'none') {
...
async function bootstrapSupabase() {
  ...
  // 1. Instant Zero-Wait Initialization from bundled config
  const bundledUrl = (window.SUPABASE_URL || '').trim();
  const bundledKey = (window.SUPABASE_ANON_KEY || '').trim();
  if (bundledUrl && bundledKey && !supabaseClient) {
    try {
      supabaseClient = lib.createClient(bundledUrl, bundledKey);
      console.log('[CrickEye] Supabase initialized immediately from bundled assets');
    } catch (e) {
      console.warn('[CrickEye] Immediate Supabase init error:', e);
    }
  }

  // 2. Fetch remote /api/public-config with a strict 2s AbortController timeout (prevents freeze)
  try {
    const controller = typeof AbortController !== 'undefined' ? new AbortController() : null;
    const timeoutId = controller ? setTimeout(() => controller.abort(), 2000) : null;
    const res = await fetch(`${getApiBaseUrl()}/api/public-config`, {
    ...
    if (res.ok) {
      ...
      if (url && key && (!supabaseClient || url !== bundledUrl)) {
        supabaseClient = lib.createClient(url, key);
        console.log('[CrickEye] Supabase client updated from /api/public-config');
      }
    }
  } catch (e) {
    console.warn('[CrickEye] /api/public-config fast fallback (bundled config used):', e?.message || e);
  }

  // 3. Final fallback check
  if (!supabaseClient && bundledUrl && bundledKey) {
    supabaseClient = lib.createClient(bundledUrl, bundledKey);
  }
}
...
function renderSessionReport(analysis) {
  ...
  container.innerHTML = `
    ...
    <div class="report-highlight-row">
      <div class="report-highlight"><div class="report-highlight-icon"><span class="material-symbols-outlined" style="color:#F97316;">whatshot</span></div><div><div class="report-highlight-title">BEST SHOT</div><div class="report-highlight-val" style="color:${bc}">#${best.shot_num} ${(SHOT_LABELS[best.label]||best.label||'—').toUpperCase()}</div><div class="report-highlight-sub">Score: ${best.shot_score}/10 · ${best.shot_quality} · ${best.timestamp}</div></div></div>
      <div class="report-highlight"><div class="report-highlight-icon"><span class="material-symbols-outlined" style="color:#EAB308;">warning</span></div><div><div class="report-highlight-title">WORST SHOT</div><div class="report-highlight-val" style="color:${wc}">#${worst.shot_num} ${(SHOT_LABELS[worst.label]||worst.label||'—').toUpperCase()}</div><div class="report-highlight-sub">Score: ${worst.shot_score}/10 · ${worst.shot_quality} · ${worst.timestamp}</div></div></div>
    </div>
    <div class="report-section-title">SESSION TREND${sm.fatigueDetected?' &nbsp;<span class="fatigue-tag"><span class="material-symbols-outlined" style="font-size:13px;vertical-align:middle;">bolt</span> FATIGUE DETECTED</span>':''}</div>
...
function replaceAnalysisCards(analysis) {
  ...
  const ICONS={HIGH:'warning',MEDIUM:'bolt',LOW:'info'};
  ...
  alerts.slice(0,cards.length).forEach((a,i)=>{
    ...
    c.querySelector('.analysis-icon').innerHTML=`<span class="material-symbols-outlined">${ICONS[a.severity]||'info'}</span>`;
...
function wireModalDismissals() {
  ...
  document.addEventListener('keydown', (e) => {
    ...
    closeModal(document.getElementById('serverSettingsModal'));
  });
}
...
async function login() {
  ...
  // Instant UI feedback to prevent multiple clicks and clarify loading state
  const origBtnText = gatePrimaryBtn ? gatePrimaryBtn.textContent : 'Login';
  if (gatePrimaryBtn) {
    gatePrimaryBtn.disabled = true;
    gatePrimaryBtn.textContent = 'Logging in...';
  }
  setGateMessage('Authenticating with CrickEye...', false);
  ...
  try {
    ...
    setGateMessage('Loading player profile...', false);
    ...
    if (gatePrimaryBtn) {
      gatePrimaryBtn.disabled = false;
      gatePrimaryBtn.textContent = origBtnText;
    }
  }
}
...
async function sha256HexFromFile(file) {
  if (!file) return null;
  // For small files (< 4MB), digest entire file.
  // For larger recordings, slice head + tail + metadata to prevent mobile WebViews from crashing / freezing
  let targetBlob;
  if (file.size <= 4 * 1024 * 1024) {
    targetBlob = file;
  } else {
    const head = file.slice(0, 2 * 1024 * 1024);
    const tail = file.slice(Math.max(0, file.size - 512 * 1024));
    const meta = new Blob([`${file.size}_${file.name}_${file.lastModified || 0}`]);
    targetBlob = new Blob([head, tail, meta]);
  }
  const buf = await targetBlob.arrayBuffer();
...
function scheduleWsReconnect() {
  ...
  if (document.hidden) return; // Do not wake CPU or radio while app/tab is backgrounded
  wsReconnectTimer = setTimeout(() => {
    connectWebSocket();
  }, wsReconnectDelay);
  // Exponential backoff capped at 15 seconds to prevent phone overheating on dead endpoints
  wsReconnectDelay = Math.min(15000, Math.round(wsReconnectDelay * 1.5));
}
...
function connectWebSocket() {
  ...
  if (document.hidden) return; // Skip connection attempt if app is in background
  updateWsStatus('connecting');

  const candidates = getCandidateServerUrls();
  const base = candidates[wsCandidateIdx % candidates.length] || getApiBaseUrl();
  let wsUrl;
  if (base.startsWith('https:')) {
    ...
    const host = window.location.host || '10.253.13.192:8000';
  ...
  try {
    ...
    console.log(`[CrickEye] Connecting WebSocket to: ${wsUrl}`);
    ...
    ws.onopen    = () => {
      ...
      console.log(`[CrickEye] WebSocket CONNECTED successfully to: ${wsUrl}`);
    };
    ...
    ws.onclose   = () => {
      ...
      if (!connectionSuccess) {
        // Rotate candidate on failed initial handshake so we test Wi-Fi and USB alternates
        wsCandidateIdx = (wsCandidateIdx + 1) % candidates.length;
      }
      scheduleWsReconnect();
    };
    ws.onerror   = (e) => {
      console.warn('[CrickEye] WS connection failed:', wsUrl);
    };
  } catch (e) {
    console.warn('[CrickEye] WS create error:', e);
    wsCandidateIdx = (wsCandidateIdx + 1) % candidates.length;
    scheduleWsReconnect();
  }
}
...
function renderLiveProcShotCard(shot) {
  ...
  const html = `
    <div class="proc-shot-header">
      <span class="proc-shot-pill" style="border-color:${color}; color:${color};"><span class="material-symbols-outlined proc-pill-icon">bolt</span> SHOT #${shot.shot_num} ANALYSED</span>
    ...
    <div class="proc-shot-main">
      ...
      <div class="proc-shot-tags">
        ${speedVal ? `<span class="proc-tag-speed"><span class="material-symbols-outlined proc-pill-icon">speed</span> ${speedVal}</span>` : ''}
...
function onComplete(msg, opts = {}) {
  ...
  // Flush dashboard to build timeline and UI elements first
  flushDashboard();
  ...
  if (state.pendingSpokes.length > 0) {
    ...
    // Keep first shot eligible for natural playback trigger
    state.drawnSpokes.delete(firstSpoke.shot_num);
  }
  ...
  setTimeout(() => {
    const base = msg.output_video.startsWith('/')
      ? `${getApiBaseUrl()}${msg.output_video}`
      : msg.output_video;
...
function flushDashboard() {
  ...
  state.shotLog.forEach(shot => {
    ...
    const isPlayed = state.drawnSpokes.has(shot.shot_num);
    el.className = `shot-circle ${shot.label||''} ${isPlayed ? 'played' : ''}`;
    ...
    el.addEventListener('click', () => {
      const spoke = state.pendingSpokes.find(s => s.shot_num === shot.shot_num);
      if (spoke && video && video.duration) {
        const origDur = state.originalVideoDuration || video.duration;
        const scale = video.duration / origDur;
        video.currentTime = Math.max(0, spoke.timestamp_sec * scale - 0.2);
        drawSpokeNow(spoke);
      }
    });
...
function playBeep(freq = 440, duration = 0.12, type = 'sine') {
  try {
    ...
    triggerHaptic(Math.min(300, Math.max(50, Math.round(duration * 1000))));
  } catch (e) {}
}

function getSupportedMimeType() {
  // Prioritize hardware-accelerated MP4 (required for iOS Safari and eliminates software encoding heat on Android)
  const types = [
...
function buildStartPanel(options = {}) {
  ...
  panel.innerHTML = `
    <div class="start-header">
      ...
      <div class="start-tabs">
        <button type="button" class="start-tab ${defaultTab === 'upload' ? 'active' : ''}" id="tabUpload"><span class="material-symbols-outlined start-tab-icon">cloud_upload</span> Upload File</button>
        <button type="button" class="start-tab ${defaultTab === 'webcam' ? 'active' : ''}" id="tabWebcam"><span class="material-symbols-outlined start-tab-icon">videocam</span> Live Web Cam</button>
      </div>
    </div>
    ...
    <div class="start-tab-content" id="uploadTabContent" style="${defaultTab === 'upload' ? 'display:flex' : 'display:none'}">
      <div class="start-form">
        ...
        <div class="upload-zone" id="uploadZone">
          <div class="upload-zone-icon"><span class="material-symbols-outlined upload-mui-icon" style="font-size:42px; color:var(--cyan);">upload_file</span></div>
        ...
        <button class="start-btn" id="startBtn" disabled><span class="material-symbols-outlined btn-mui-icon">play_arrow</span> RUN CRICKEYE PIPELINE</button>
      </div>
    </div>
    ...
    <div class="start-tab-content" id="webcamTabContent" style="${defaultTab === 'webcam' ? 'display:flex' : 'display:none'}">
      ...
      <div class="webcam-toolbar">
        <div class="webcam-device-wrap">
          ...
          <button type="button" id="webcamFlipCameraBtn" class="webcam-tool-btn" title="Flip Camera (Front/Back)"><span class="material-symbols-outlined tool-mui-icon">cameraswitch</span> Flip</button>
          <button type="button" id="webcamToggleGuideBtn" class="webcam-tool-btn" title="Toggle Stance Guide"><span class="material-symbols-outlined tool-mui-icon">filter_center_focus</span> Guide</button>
        </div>
      ...
      <div class="webcam-actions" id="webcamLiveActions">
        <button type="button" class="start-btn start-btn--record" id="webcamAutoRecordBtn">
          <span class="material-symbols-outlined btn-mui-icon">fiber_manual_record</span> RECORD DELIVERY (3s COUNTDOWN)
        </button>
        <div class="webcam-secondary-actions">
          <button type="button" class="webcam-manual-btn" id="webcamManualRecordBtn"><span class="material-symbols-outlined tool-mui-icon">radio_button_checked</span> Manual Record</button>
          <button type="button" class="webcam-cancel-btn" id="webcamStopRecordBtn" style="display:none"><span class="material-symbols-outlined tool-mui-icon">stop_circle</span> Stop &amp; Analyze</button>
        </div>
  ...
  async function getStreamWithFallback(selectedDevId) {
    ...
    const preferredFacing = localStorage.getItem('crickeye_camera_facing') || 'environment';
  ...
  const flipCameraBtn = document.getElementById('webcamFlipCameraBtn');
  flipCameraBtn?.addEventListener('click', () => {
    const cur = localStorage.getItem('crickeye_camera_facing') || 'environment';
    const nxt = cur === 'environment' ? 'user' : 'environment';
    localStorage.setItem('crickeye_camera_facing', nxt);
    triggerHaptic(60);
    initWebcam();
  });
  ...
  function recordWebcamStream(durationSec, onFinish) {
    ...
    currentMediaRecorder.onstop = () => {
      ...
      if (!blob || blob.size === 0) {
        ...
        if (autoBtn) { autoBtn.disabled = false; autoBtn.innerHTML = '<span class="material-symbols-outlined btn-mui-icon">fiber_manual_record</span> RECORD DELIVERY (3s COUNTDOWN)'; }
    ...
    currentMediaRecorder.start(1000); // 1-second chunks eliminate GC thrashing and heat on mobile
  }
  ...
  autoRecordBtn?.addEventListener('click', async () => {
    ...
    if (!currentWebcamStream) {
      ...
      autoRecordBtn.innerHTML = '<span class="material-symbols-outlined btn-mui-icon">hourglass_empty</span> STARTING CAMERA…';
      await initWebcam();
      autoRecordBtn.disabled = false;
      autoRecordBtn.innerHTML = '<span class="material-symbols-outlined btn-mui-icon">fiber_manual_record</span> RECORD DELIVERY (3s COUNTDOWN)';
    }
...
function onVideoTimeUpdate() {
  ...
  const scale   = video.duration / origDur;
  const AHEAD   = 0.4;
  if (state._lastVideoTime !== undefined && t < state._lastVideoTime - 1.0) {
    state.drawnSpokes.clear();
    document.querySelectorAll('.shot-circle').forEach(el => el.classList.remove('played', 'active'));
  }
  state._lastVideoTime = t;
  state.pendingSpokes.forEach(spoke => {
...
function init() {
  ...
  setAuthMode('login');
  initAuth();
  console.log('[CrickEye Pro v6.6 — Mobile & Wireless APK Edition] Initialised.');
}

function initServerSettingsModal() {
  const modal = document.getElementById('serverSettingsModal');
  const btn = document.getElementById('headerServerBtn');
  const input = document.getElementById('serverUrlInput');
  const pingBtn = document.getElementById('btnTestServerConnection');
  const pingResult = document.getElementById('serverPingResult');
  const saveBtn = document.getElementById('btnSaveServerSettings');
  const statusText = document.getElementById('headerServerStatusText');

  const presetWifi = document.getElementById('presetWifi');
  const presetLocalhost = document.getElementById('presetLocalhost');
  const presetCloudflare = document.getElementById('presetCloudflare');
  const presetEmulator = document.getElementById('presetEmulator');
  const presetOrigin = document.getElementById('presetOrigin');

  const facingRadios = document.querySelectorAll('input[name="cameraFacingMode"]');
  const hapticToggle = document.getElementById('hapticFeedbackToggle');

  function updateStatusBadge() {
    const base = getApiBaseUrl();
    const iconEl = document.getElementById('headerServerIcon');
    if (statusText) {
      try {
        const u = new URL(base);
        if (u.hostname === 'localhost' || u.hostname === '127.0.0.1') {
          statusText.textContent = 'USB';
          if (iconEl) iconEl.textContent = 'usb';
        } else if (u.hostname === '10.253.13.192' || u.hostname.startsWith('10.') || u.hostname.startsWith('192.168.')) {
          statusText.textContent = 'Wi-Fi';
          if (iconEl) iconEl.textContent = 'wifi';
        } else if (u.hostname.includes('trycloudflare')) {
          statusText.textContent = 'Cloud';
          if (iconEl) iconEl.textContent = 'cloud';
        } else {
          statusText.textContent = u.hostname;
          if (iconEl) iconEl.textContent = 'dns';
        }
      } catch {
        statusText.textContent = 'Server';
        if (iconEl) iconEl.textContent = 'router';
      }
    }
  }
  updateStatusBadge();

  if (window.AndroidBridge || (navigator.userAgent && (navigator.userAgent.includes('Android Native') || navigator.userAgent.includes('CrickEyeProMobile')))) {
    document.body.classList.add('in-android-apk');
  }

  btn?.addEventListener('click', () => {
    if (input) input.value = getApiBaseUrl();
    if (pingResult) pingResult.textContent = '';
    const currentFacing = localStorage.getItem('crickeye_camera_facing') || 'environment';
    facingRadios.forEach(r => { r.checked = (r.value === currentFacing); });
    if (hapticToggle) hapticToggle.checked = (localStorage.getItem('crickeye_haptics') !== 'false');
    openModal(modal);
  });

  presetWifi?.addEventListener('click', () => { if (input) input.value = 'http://10.253.13.192:8000'; });
  presetLocalhost?.addEventListener('click', () => { if (input) input.value = 'http://localhost:8000'; });
  presetCloudflare?.addEventListener('click', () => { if (input) input.value = 'https://suggestion-apartment-plane-yet.trycloudflare.com'; });
  presetEmulator?.addEventListener('click', () => { if (input) input.value = 'http://10.0.2.2:8000'; });
  presetOrigin?.addEventListener('click', () => {
    if (input) input.value = window.location.origin && window.location.origin !== 'null' && window.location.protocol !== 'file:' ? window.location.origin : 'http://10.253.13.192:8000';
  });

  pingBtn?.addEventListener('click', async () => {
    const testUrl = (input?.value || '').trim().replace(/\/+$/, '');
    if (!testUrl) {
      if (pingResult) { pingResult.className = 'server-ping-status err'; pingResult.textContent = 'Please enter a server URL'; }
      return;
    }
    if (pingResult) { pingResult.className = 'server-ping-status'; pingResult.textContent = 'Pinging ' + testUrl + '…'; }
    try {
      const ctrl = new AbortController();
      const tid = setTimeout(() => ctrl.abort(), 6000);
      const res = await fetch(testUrl + '/api/public-config', { signal: ctrl.signal });
      clearTimeout(tid);
      if (res.ok) {
        if (pingResult) { pingResult.className = 'server-ping-status ok'; pingResult.textContent = '✓ Connected! FastAPI backend is online and responding.'; }
      } else {
        if (pingResult) { pingResult.className = 'server-ping-status err'; pingResult.textContent = `Server responded with HTTP ${res.status}`; }
      }
    } catch (e) {
      if (pingResult) { pingResult.className = 'server-ping-status err'; pingResult.textContent = 'Connection failed: ' + (e.name === 'AbortError' ? 'Timed out (6s)' : (e.message || 'Cannot reach server')); }
    }
  });
  ...
  saveBtn?.addEventListener('click', async () => {
    ...
    if (newUrl) {
      localStorage.setItem('crickeye_server_url', newUrl);
    } else {
      localStorage.removeItem('crickeye_server_url');
    }

    let selectedFacing = 'environment';
    facingRadios.forEach(r => { if (r.checked) selectedFacing = r.value; });
    localStorage.setItem('crickeye_camera_facing', selectedFacing);

    if (hapticToggle) {
      localStorage.setItem('crickeye_haptics', hapticToggle.checked ? 'true' : 'false');
    }
    ...
    if (window.AndroidBridge && typeof window.AndroidBridge.showToast === 'function') {
      window.AndroidBridge.showToast('Server connection updated: ' + (newUrl || 'Default'));
    }
  });
}
  ...
  navItems.forEach(item => {
    item.addEventListener('click', () => {
      ...
      if (target === 'capture') {
        ...
        document.querySelector('.live-feed-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else if (target === 'score') {
        document.querySelector('.playcard-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else if (target === 'pitch') {
        document.querySelector('.analytics-split-row')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else if (target === 'biomech') {
        document.querySelector('.biomech-section')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else if (target === 'sessions') {
        document.querySelector('.user-sessions-panel')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
      } else if (target === 'settings') {
        const mod = document.getElementById('serverSettingsModal');
...
// ── Mobile Lifecycle & Resource Guard ───────────────────────
document.addEventListener('visibilitychange', () => {
  if (document.hidden) {
    // When app is minimized or screen turned off, kill camera to stop heating and battery burn
    ...
    // When returning to foreground, reconnect WS if disconnected
    if (!ws || ws.readyState !== WebSocket.OPEN) {
